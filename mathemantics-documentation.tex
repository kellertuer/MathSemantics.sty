...
\begin{document}
    ...
    This package aims to provide some commands for ease of use in mathematics.
    ...
    The parts of this packages (per section) are stored in separate files and can be loaded separately.
    If one part depends on another package, this is mentioned in the beginning of the section.

    \section{Package Options}
        \label{sec:package-options}
        \begin{commandlist}
            \item[shortbb] use shorter notations for the bold math fonts, \codeCommand{\R} for \codeCommand{\bbR}. (planned)
        \end{commandlist}
    ...
    The \file{mathemantics-syntax.sty} package provides mainly symbols and short commands, which can be used in semantic definitions for ease of notation.
    ...
    \begin{commandlist}
        ...
        \item[bbC, bbK, bbN, bbQ, bbR, bbS, bbZ] mathematical sets \par\mathCodeExample{\bbC,\bbK,\bbN,\bbQ,\bbR,\bbS,\bbZ}
    \end{commandlist}
    ...
    \begin{commandlist}
        ...
        \item[restr] restriction/evaluation.
            ...
            \par\mathCodeExample{\restr[auto]{\frac{\d x}{\d t}(f \circ \gamma)(t)}{t=0}}
      ...
      \item[setDef] define a set, where \codeCommand{\setMid} serves as the center divider.
        ...
        \item[setMid] divider within \codeCommand{\setDef} (set definitions).
            This defaults to \codeCommand{|}.
            % To produce a colon as divider, use \codeCommand{\renewcommand{\setMid}{.\colon}}.
...
\begin{commandlist}
	...
	\item[parallelTransportSymbol]\hspace{4em}the symbol to use within \codeCommand{\parallelTransport} and \codeCommand{\parallelTransportDir}
	...
	\item[secondCovariantDerivative]\hspace{4em}is the second-order covariant derivative.
	...
	\item[secondCovariantDerivativeSymbol]\hspace{4em}the symbol used for the second covariant derivative \codeCommand{\secondCovariantDerivative}.
	...
	\item[vectorTransportSymbol]\hspace{4em}the symbol to use within \codeCommand{\vectorTransport} and \codeCommand{\vectorTransportDir}
...
The semantic file \file{mathemantics-optimization.sty} collects definitions and notations related to optimization.
\begin{commandlist}
